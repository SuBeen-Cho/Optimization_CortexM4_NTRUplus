...
/* [PORT] 2-arg (out,in) interface to match the memory-opt poly.c layer.
 * Wrappers copy a->r then run the in-place asm driver on r. */
void ntt(int16_t r[NTRUPLUS_N], const int16_t a[NTRUPLUS_N]);
void invntt(int16_t r[NTRUPLUS_N], const int16_t a[NTRUPLUS_N]);

// asm: radix-2 CT layers (step 64..4) + Barrett pass of ntt(); zp = &zetas[6]
