...
static void ntt_inplace(int16_t r[NTRUPLUS_N])   /* [PORT] in-place asm driver */
{
	...
	// radix-3 (OMEGA) layers in asm (step 144..48); KAT-gated.
...
static void invntt_inplace(int16_t r[NTRUPLUS_N])   /* [PORT] in-place asm driver */
{
	...
	/* [FIX-ASM] asm INVNTT leaves result one Montgomery factor low (asm =
	 * clean*R^-1), previously paired with the buggy R^0 basemul. Pass clean's
	 * final-scale constants (-33, -66) instead of (NINV, 2NINV) so the asm INVNTT
	 * output matches clean byte-for-byte (invntt_scale == clean fqmul(-33,..)/
	 * fqmul(-66,..)). */
...
* Returns:     none.
**************************************************/
#if 0 // replaced by packed asm in ntt_basemul.S (X^3, KAT byte-identical)
