...
static void ntt_inplace(int16_t r[NTRUPLUS_N])   /* [PORT] in-place asm driver */
{
	...
	// radix-3 (OMEGA) layers in asm (step 192..64); KAT-gated.
	for (int step = NTRUPLUS_N/6; step >= 64; step = step/3)
	{
		ntt_radix3_layer(r, &zetas[k], NTRUPLUS_N, step);
		k += 2 * (NTRUPLUS_N / (3 * step));
	}

	// radix-2 Cooley-Tukey layers (barrett inside butterfly), in asm
	// (bit-identical to the C reference; KAT-gated). zetas index k==18 here.
...
static void invntt_inplace(int16_t r[NTRUPLUS_N])   /* [PORT] in-place asm driver */
{
	...
	{
		invntt_radix3_layer(r, &zetas[k], NTRUPLUS_N, step, 1);
		k -= 2 * (NTRUPLUS_N / (3 * step));
	}

	/* [FIX-ASM] The asm INVNTT layers leave the result one Montgomery factor low
	 * (asm = clean * R^-1), which the old asm convention paired with the buggy
	 * R^0 basemul. To restore clean's domain at INVNTT output, pass clean's
	 * final-scale constants (-33 = ninv, -66 = 2ninv for n/d=384) instead of the
	 * (NINV=ninv*R, 2NINV) Montgomery-form constants. invntt_scale computes
	 * mont(c*(T1-T2))/mont(c*T2) == clean's fqmul(-33,..)/fqmul(-66,..) loop. */
	invntt_scale(r, NTRUPLUS_N, -33, -66);
}
...
**************************************************/
#if 0 // baseinv now in ntt_baseinv.S (asm, KAT byte-identical)
...
* Returns:     none.
**************************************************/
#if 0  // basemul now in ntt_basemul.S (packed asm)
...
* Returns:     none.
**************************************************/
#if 0  // basemul_add now in ntt_basemul.S (packed asm)
...
/*************************************************
* [PORT] 2-arg (out,in) wrappers over the in-place asm drivers.
* Host twin (../m4opt_mem_hosttwin) verified byte-identical KAT vs clean.
**************************************************/
